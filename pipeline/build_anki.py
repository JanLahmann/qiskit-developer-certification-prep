...
STUDY_BOOK_MDX = REPO_ROOT / "site" / "docs" / "study-book.mdx"
...
**No Anki on your device?** The same content also ships as an [EPUB study book for Kindle & e-readers](/docs/study-book).
...
| Kindle & e-ink readers | the [EPUB study book](/docs/study-book) | free | No Anki apps run on e-ink; the EPUB rebuilds the question→answer flow |
...
def render_study_book_mdx() -> str:
    ...
    return f"""---
sidebar_position: 12
title: Study book (Kindle & e-readers)
description: The CertiQ study layer and full verified question bank as an EPUB book for Kindle and any e-reader.
---
...
# Study book (Kindle & e-readers)

All of CertiQ, packaged for offline reading. **Part I** is the study layer: a short primer per exam objective, the must-know facts and traps, and an exam checklist per section. **Part II** is the full question bank as a drill — each question on its own page with the answer on the page *after* it, so nothing is spoiled while you think.

The book regenerates from the verified bank at every site build, so re-downloading always gives you the latest content.
...
Kobo, Boox, PocketBook and Apple Books open the EPUB directly — just download and open, no Amazon step needed.

## Prefer interactive drilling?

The same questions ship as [Anki flashcards](/docs/flashcards) with spaced repetition, and every practice tool on this site draws from the same verified bank.

:::note Unofficial
CertiQ is an independent community project. This book is AI-generated from the public exam objectives and open documentation, then execution-verified against a pinned Qiskit 2.x stack — it is **not** affiliated with, endorsed by, or sponsored by IBM, and never derived from real exam content.
:::
"""
...
def write_flashcards_mdx(rows: list[dict], all_row: dict) -> None:
    ...
    STUDY_BOOK_MDX.write_text(render_study_book_mdx(), encoding="utf-8")
